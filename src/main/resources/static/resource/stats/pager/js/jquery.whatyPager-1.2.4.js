/**
 * @todo: jQuery Ajax通用分页插件 jquery.whatyPager-1.2.3.js
 *        已测试支持jquery-1.9.1+
 *        b优化：
 ...
 *                    2）、数据准备完毕后，集中渲染分页栏
 *            4、（v_1.2.3 已优化）分页栏显示控制和加入数据格式解析回调函数
 *            5、（v_1.2.4 已优化）分页异步加载分页栏未初始化bug修复；各方法优化
 *
 *        后期优化路线：
...
(function ($) {
    ...
    var methods = {
         ...
         * 初始化分页
         */
        initPager: function (element, opts) {
            ...
            //默认设置对象与初始化散列参数对象合并
            var _dfs = $.extend(true, {}, $.defaultSetting);
            var options = $.extend(true, _dfs, opts || {});
            var testOb = {yin:"ddddd", "mmm":element};
            $(element).data('pageSettings', options);
            $(element).data('test', _dfs);

            //初始化分页栏html
            methods.initPaperToolBar.apply($(element), [options]);

            //执行分页查询
            methods._pageSubmit.apply($(element));

            //渲染分页栏信息数据，并更新连续分页
            // methods._showPagerToolBarData.apply(this);

            //初始化按钮事件处理函数
            methods._initPageNum.apply($(element));
            methods._initPageBtn.apply($(element));
            methods._initPageSizeSelect.apply($(element));		//初始化每页显示条数下拉选项
        ...
        _pageSubmit: function () {
            ...
            /*if (options.contentType.indexOf("application/json") > -1) {
                if (searchParam != null) {
                    var obj = JSON.parse(searchParam);
                    obj.curPage = options.curPageNum ? parseInt(options.curPageNum) : 1;
            ...
            $.ajax({
                ...
                data: JSON.parse(searchParam),	//条件查询参数，可能是string、Array、json、或者jsonArray
                type: 'post',
                ...
                success: function (pageData) {
                    // alert($(this).attr("id")+"====="+$(_this).attr("id"))
                    ...
                    if (typeof options.parsePageData === "function") {	//返回的数据格式与插件能解析的格式不一致时，使用用户自定义的数据解析函数，处理数据使其与插件要求的数据格式一致，格式：{'curPage':'1', 'pageSize':'10', 'totalRow':'102', 'totalPage':'11', 'data':[{...},{...}]}（注意除了'data'以外的其他key名称都必须保持一致
                        var formatPageData = options.parsePageData.call(_this, pageData);
                    ...
                    if (options.dataType.toLowerCase() === 'html' && typeof options.setTotalCount === "function") {
                        totalCount = options.setTotalCount.call(_this);
                    ...
                    //console.log("162::" + JSON.stringify(options));
                    $(_this).data('pageSettings', options);

                    //计算更新连续分页页码数据
                    methods._updateContinuePagerData.apply(_this);

                    //渲染连续分页，存入到options中
                    methods._appendContinuePager.apply(_this);
        ...
        _updateContinuePagerData: function () {
            ...
            //console.log("234::" + $(this).attr('id'));
            //console.log("234::" + JSON.stringify($(this).data('pageSettings')));
            ...
            $(this).data('pageSettings', options);
            //console.log("350::" + $(this).attr('id'));
            //console.log("350::" + JSON.stringify($(this).data('pageSettings')));
            // });
        },
        ...
        _appendContinuePager: function () {
            ...
            $(this).data('pageSettings', options);
            //console.log("400::" + $(this).attr('id'));
            //console.log("400::" + JSON.stringify($(this).data('pageSettings')));
        },
        ...
        initPaperToolBar: function (optsions) {
            ...
            var pageObject = undefined == optsions.pageObject ? '' : optsions.pageObject;

            $(this).empty();
            var paperToolBar = ""
                + '<div id="pager_plugin' + pageObject + '">'
                + '	<div id="pager_data' + pageObject + '" class="recordtotal">'
                + '		<input type="hidden" id="currentPage' + pageObject + '" value="1"/>'
                + '		<input type="hidden" id="pageSize' + pageObject + '"/>'
                + '	</div>'
                + '	<div id="pagelist' + pageObject + '" class="paging">'
                + '		<div>'
                + '			<div><a href="javascript:void(0);" id="start' + pageObject + '" class="p1">首页</a></div>'
                + '			<div><a href="javascript:void(0);" id="previous' + pageObject + '" class="p2">上一页</a></div>'
                + '		</div>'
                + '		<div id="toolBar_pageLink' + pageObject + '">'
                + '			<div><a href="javascript:void(0);" class="p3 p_current">1</a></div>'
                + ' 			<!-- <div><a href="javascript:void(0);" class="p3">2</a></div> -->'
                + '		</div>'
                + '    	<div>'
                + '    		<div><a href="javascript:void(0);" id="next' + pageObject + '" class="p2">下一页</a></div>'
                + '    		<div><a href="javascript:void(0);" id="end' + pageObject + '" class="p1">尾页</a></div>'
                + '    		<div id="toolBar_curPage' + pageObject + '"><span id="showCurPage' + pageObject + '" class="font-color-red">1</span></div>'
                + '    		<div id="toolBar_totalPage' + pageObject + '">/<span id="totalPage' + pageObject + '" class="font-color-red">1</span>页</div>'
                + '    		<div id="toolBar_rowCount' + pageObject + '">&nbsp;共<span id="rowCount' + pageObject + '" class="font-color-red">0</span>条</div>'
                + '    		<div id="toolBar_jumpPage' + pageObject + '">&nbsp;<input type="text" id="pageNum' + pageObject + '" value=""/><a href="javascript:void(0);" id="jumpPageNum' + pageObject + '" class="p4 jump_btn">跳页</a></div>'
                + '    		<div id="toolBar_pageSizeSelect' + pageObject + '">&nbsp;<span style="float:left;*margin-top:-24px;color:#5c5c5c;">每页</span>&nbsp;&nbsp;<select id="pageSize_select' + pageObject + '" style="float:left;*margin-top:-24px;"><option value="10">10</option></select>&nbsp;&nbsp;<span style="float:left;*margin-top:-24px;color:#5c5c5c;">条</span></div>'
                + '    	</div>'
                + '	</div>'
                + ' </div>';
        ...
        _initPageNum: function () {
            var _this = this;
            var options = $(this).data('pageSettings');
            $(options.pageLinkSelector).off("click", "div > a");
            $(options.pageLinkSelector).on("click", "div > a", function () {
                var _pageNum = $(this).attr("_continue_pagenum_");
                options = $.extend(options, {curPageNum: _pageNum, pageJumpType: null});
                $(_this).data('pageSettings', options);

                methods._pageJump.apply(_this);
            });
        },
        ...
        _initPageBtn: function () {
            ...
            var _this = this;
            var options = $(this).data('pageSettings');
            //console.log("513::" + JSON.stringify(options));
            //console.log("513::" + _this.attr('id'));
            $(options.firstPage + "," + options.prePage + "," + options.nextPage + "," + options.lastPage + "," + options.jumpPageNum).on("click", function () {
                ...
                //console.log("516::" + JSON.stringify(options));
                //console.log("516::" + _this.attr('id'));
                $(_this).data('pageSettings', options);
        ...
        _pageJump: function () {
            ...
            var pageObject = options.pageObject;
            pageObject = undefined == pageObject ? '' : pageObject;

            // 设置跳转页面开始记录数
            if (btnType == "start" + pageObject) {

                currentPage = 1;
            } else if (btnType == "previous" + pageObject) {
            ...
            } else if (btnType == "next" + pageObject) {
            ...
            } else if (btnType == "end" + pageObject) {

                currentPage = totalPage;
            } else if (btnType == "jumpPageNum" + pageObject) {		//跳页
            ...
            $(this).data('pageSettings', options);
            //console.log("579::" + $(this).attr('id'));
            //console.log("579::" + JSON.stringify($(this).data('pageSettings')));
        ...
        _initPageSizeSelect: function () {
            ...
            if (options.isShowPageSizeSelectToolBar) {	//如果显示页大小下拉选项，则初始化下拉选项
                ...
                $(options.pageSizeSelectSelector).on("change", function () {
                    ...
                    var options = $(_this).data('pageSettings');
                    ...
                    methods._pageSubmit.apply(_this);
                });
            }
            // });
        },
        ...
        reloadPager: function (reloadOptions) {
            ...
            $(this).data('pageSettings', options);
            //console.log("655::" + $(this).attr('id'));
            //console.log("655::" + JSON.stringify($(this).data('pageSettings')));
            ...
            $(this).data('pageSettings', options);
            //console.log("707::" + $(this).attr('id'));
            //console.log("707::" + JSON.stringify($(this).data('pageSettings')));
            methods._pageSubmit.apply(this);
            // });
        },
   ...
   /* $.fn.whatyPager = function () {
    ...
    $.fn.whatyPager = function (options) {
        var _this = $(this);
        new WhatyPager(_this, options);
    };

    var WhatyPager = function (_this, options) {

        var method = arguments[1];	//获取参数列表中第一个字符串参数，即为要调用的方法名
        if (methods[method]) {	 	// 检验方法是否存在
...
//		pageJumpType: null, 						//翻页类型，插件儿内部跳转使用
        ...
        async: true,								// true：异步加载；false：同步加载
        pagerCallHandel: function (pageData, searchParam) {
        },	//分页数据处理回调函数；1、dataType:'html'时，该回调函数参数返回的是分页渲染后的html页面片段；2、dataType:'json'时，回调函数参数返回的是分页数据对象（如：分页数据对象pager的json数据或者其他对象模型的json格式数据）
