$(function () {
     ...
     * 监听删除接收人事件
     */
    $("#receive_people_list").on("click", ".tag-del", function () {
        // 自身删除
        $(this).parent().remove();
     ...
     *
     */
    $("#manager_info").on("click", ".tag-menu-li", function () {
        var _this = $(this);

        var uid = $('.tag-add').parent().find('#' + _this.attr('uid'));
        if(uid.length < 1){
            // 添加新标签
            addTag();

            // 获取最新添加的标签，并赋予相应的ID
            var addedTag = $('.tag-add').prev().attr('id', _this.attr('uid'));

            // 获取选中的目录标签文字内容
            var val = $(this).find('.tag-menu-name').text();
            addedTag.find('.tag-name').text(val);
        }
    });
