...
Ext.onReady(function () {
        ...
        function onContextMenu(ts, item, e) {
            if (!menu) { // create context menu on first right click
                menu = new Ext.menu.Menu([{
				    ...
				    * , { id : tabs.id + '-collect', text :
					* '收藏', iconCls : "refresh", handler :
					* function(){ collectNode(ctxItem);
					* //alert(this.id); } }
					*/
                ]);
            }
    ...
    function collectNode(currentNode) {
        Ext.Ajax.request({
            url: '/sso/managerMenu_collectNode.action', // 请求的地址
            method: "POST",
            params: {
            ...
            success: function (response, option) {
                ...
                if (response.success == 'true') {
                    ...
                    Ext.MessageBox.alert('提示', '收藏失败1!');
                }
            },
            failure: function (response, option) {
                Ext.MessageBox.alert('提示', '收藏失败2!');
            }
        });
    }
    ...
    var loader = new Ext.tree.TreeLoader({
        url: '/sso/peExttree_treeJson.action'
    });
        ...
        listeners: {
            'click': function (node, event) {
                if (node.isLeaf() && editorFlag != true) {
                    // 为叶子节点时，点击进入链接
                    var nname = node.text;
                    var nhref = node.attributes.href;
                    event.stopEvent();
    ...
    if (urlArr.length == 0) {
        urlArr = [{
            nid: '1001',
            name: '课程列表',
            url: '/manager/basic/peTchCourse.action'
        }];
    }
    var _tabPanels = [];
