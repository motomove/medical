...
public class UploadResponse {
	...
	public void setCustomMessage(final String customMassage) {
		if (StringUtils.isNotEmpty(customMassage)) {
...
//				.getRequest());
//		lm.getFileRenamedWarning(newFileName)
		return new UploadResponse(EN_FILE_RENAMED_WARNING, fileUrl,
				newFileName, "File rename warning: "+newFileName);
	}
...
//		lm.getInvalidFileTypeSpecified()
		return new UploadResponse(EN_INVALID_FILE_TYPE_ERROR, "Invalid file type specified");
	}
...
//		LocalizedMessages lm = LocalizedMessages.getInstance(ThreadLocalData
//				.getRequest());
//		lm.getInvalidCommandSpecified()
		return new UploadResponse(EN_CUSTOM_ERROR, null, null, "Invalid command specified");
	}
...
//		LocalizedMessages lm = LocalizedMessages.getInstance(ThreadLocalData
//				.getRequest());
//		lm.getInvalidResouceTypeSpecified()
		return new UploadResponse(EN_CUSTOM_ERROR, null, null, "Invalid resouce type specified");
	}
...
//		LocalizedMessages lm = LocalizedMessages.getInstance(ThreadLocalData
//				.getRequest());
//		lm.getInvalidCurrentFolderSpecified()
		return new UploadResponse(EN_CUSTOM_ERROR, null, null, "Invalid current folder specified");
	}
...
//		LocalizedMessages lm = LocalizedMessages.getInstance(ThreadLocalData
//				.getRequest());
//		lm.getFileUploadDisabled()
		return new UploadResponse(EN_SECURITY_ERROR, null, null, "File upload disabled");
	}
...
//		LocalizedMessages lm = LocalizedMessages.getInstance(ThreadLocalData
//				.getRequest());
//		lm.getFileUploadWriteError()
		return new UploadResponse(EN_CUSTOM_ERROR, null, null, "File upload write error");
	}
